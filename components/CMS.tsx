...
// Helper component to handle raw JSON editing
...
const JsonEditor: React.FC<JsonEditorProps> = ({ value, onChange }) => {
    ...
    return (
        <div className="flex flex-col">
            <textarea 
                className={`w-full bg-slate-950 border ${error ? 'border-red-500' : 'border-fuchsia-900'} p-2 rounded text-xs text-fuchsia-100 h-48 md:h-64 font-mono focus:outline-none focus:border-cyan-500 transition-colors`}
                value={text} 
...
const CMS: React.FC<CMSProps> = ({ data, onSave, onClose }) => {
    ...
    const updateAIKeys = (provider: string, value: string) => {
        setLocalData({ 
            ...localData, 
            aiConfig: { 
                ...localData.aiConfig, 
                apiKeys: { ...localData.aiConfig.apiKeys, [provider]: value }
            } 
        });
    };

    const updateAIModels = (provider: string, value: string) => {
        setLocalData({ 
            ...localData, 
            aiConfig: { 
                ...localData.aiConfig, 
                models: { ...localData.aiConfig.models, [provider]: value }
            } 
        });
    };
    ...
    const renderSectionEditor = (section: SectionData) => {
        return (
            <div className="space-y-4 p-3 md:p-4 border border-slate-700 rounded bg-slate-900/50 mt-4">
                <div className="flex justify-between items-center mb-2">
                    <h4 className="text-cyan-400 font-bold text-sm">Edit Content</h4>
                    <button 
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center md:p-4">
            <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="w-full h-full md:h-[90vh] md:max-w-5xl bg-slate-900 border border-cyan-500/30 md:shadow-2xl md:rounded-lg flex flex-col overflow-hidden"
            >
                {/* Header */}
                <div className="p-3 md:p-4 border-b border-cyan-500/20 flex justify-between items-center bg-slate-900">
                    <div className="flex items-center space-x-2">
                        <Settings className="text-cyan-400 w-5 h-5" />
                        <h2 className="text-lg md:text-xl font-orbitron text-white truncate">System Configuration</h2>
                    </div>
                    <div className="flex space-x-2">
                        <button onClick={handleSave} className="flex items-center px-3 py-1.5 md:px-4 md:py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded transition-colors text-xs md:text-sm font-bold shadow-[0_0_15px_rgba(8,145,178,0.5)]">
                            <Save size={14} className="mr-1 md:mr-2" /> Save
                        </button>
                ...
                <div className="flex border-b border-cyan-500/20 bg-slate-900/50 overflow-x-auto no-scrollbar">
                    {['general', 'sections', 'ai', 'footer', 'analytics'].map((tab) => (
                        <button 
                            ...
                            className={`flex-shrink-0 px-4 md:px-6 py-3 text-xs md:text-sm font-bold tracking-wider transition-colors uppercase ${activeTab === tab ? 'bg-cyan-900/30 text-cyan-400 border-b-2 border-cyan-400' : 'text-slate-500 hover:text-slate-300'}`}
                        >
                            {tab}
                        </button>
                    ))}
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto p-4 md:p-6 scrollbar-thin scrollbar-thumb-cyan-900">
                    {activeTab === 'general' && (
                        <div className="space-y-4 md:space-y-6 max-w-2xl mx-auto">
                             <div className="bg-cyan-900/10 border border-cyan-500/30 p-3 rounded text-xs md:text-sm text-cyan-200 mb-4">
                                Configure the main landing visual parameters.
                            </div>
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Hero Title</label>
                                <input 
                            ...
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Hero Subtitle</label>
                                <input 
                            ...
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Rotating Words</label>
                                <input 
                    ...
                    {activeTab === 'ai' && (
                        <div className="space-y-6 max-w-2xl mx-auto">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {(['openai', 'gemini', 'huggingface'] as AIProvider[]).map(provider => (
                                    <button
                                        key={provider}
                                        onClick={() => updateAI('provider', provider)}
                                        className={`p-3 rounded border text-sm font-bold uppercase ${localData.aiConfig.provider === provider ? 'bg-cyan-900/50 border-cyan-400 text-cyan-300' : 'bg-slate-900 border-slate-700 text-slate-500 hover:text-slate-300'}`}
                                    >
                                        {provider}
                                    </button>
                                ))}
                            ...
                            <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-lg space-y-4">
                                <h4 className="text-cyan-400 font-bold text-sm uppercase mb-2">
                                    {localData.aiConfig.provider} Configuration
                                </h4>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        type="password"
                                        value={localData.aiConfig.apiKeys[localData.aiConfig.provider]}
                                        onChange={(e) => updateAIKeys(localData.aiConfig.provider, e.target.value)}
                                        placeholder={`Enter ${localData.aiConfig.provider} API Key`}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        value={localData.aiConfig.models[localData.aiConfig.provider]}
                                        onChange={(e) => updateAIModels(localData.aiConfig.provider, e.target.value)}
                                        className="w-full bg-slate-950 border border-slate-700 p-3 rounded text-white focus:border-cyan-500 outline-none font-mono text-xs"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">System Prompt</label>
                                <textarea 
                                    value={localData.aiConfig.systemPrompt}
                                    onChange={(e) => updateAI('systemPrompt', e.target.value)}
                                    className="w-full bg-slate-950 border border-slate-700 p-3 rounded text-white focus:border-cyan-500 outline-none h-32"
                                />
                                <p className="text-[10px] text-slate-500">Website content is automatically appended to this prompt.</p>
                            </div>
                            
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Welcome Message</label>
                                <input 
                    ...
                    {activeTab === 'footer' && (
                        <div className="space-y-4 md:space-y-6 max-w-2xl mx-auto">
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Section Title</label>
                                <input 
                            ...
                            <div className="space-y-2">
                                <label className="block text-xs md:text-sm text-cyan-400 font-bold">Message</label>
                                <textarea
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-800">
                                ...
                                <div className="space-y-2">
                                    <label className="block text-xs text-slate-400">LinkedIn URL</label>
                                    <input 
                                ...
                                <div className="space-y-2">
                                    <label className="block text-xs text-slate-400">Twitter URL</label>
                                    <input 
                                ...
                                <div className="space-y-2">
                                    <label className="block text-xs text-slate-400">Email (mailto:)</label>
                                    <input 
                    ...
                    {activeTab === 'sections' && (
                        <div className="space-y-4">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-white font-bold text-sm md:text-base">Manage Sections</h3>
                                ...
                                <div key={section.id} className={`bg-slate-950/50 border ${editingSectionId === section.id ? 'border-cyan-500' : 'border-slate-800'} rounded-lg overflow-hidden transition-colors`}>
                                    ...
                                    >
                                        <div className="flex items-center space-x-2 md:space-x-3 overflow-hidden">
                                            ...
                                            <span className={`${section.isVisible ? 'text-cyan-400' : 'text-slate-500 line-through'} font-bold text-sm truncate`}>{section.title}</span>
                                            <span className="text-[10px] text-slate-500 uppercase bg-slate-950 px-1.5 py-0.5 rounded border border-slate-800 hidden sm:inline-block">{section.type}</span>
                                        </div>
                    ...
                    {activeTab === 'analytics' && localData.analytics && (
                         <div className="space-y-6 max-w-4xl mx-auto">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-lg">
                                    ...
                                    <span className="text-3xl font-orbitron text-white">{localData.analytics.totalVisits}</span>
                                </div>
                                ...
                                <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-lg">
                                    ...
                                    <span className="text-3xl font-orbitron text-white">{localData.analytics.devices.mobile}</span>
                                </div>
                                ...
                                <div className="bg-slate-900/50 border border-slate-800 p-4 rounded-lg">
                                    ...
                                    <span className="text-3xl font-orbitron text-white">{localData.analytics.devices.desktop}</span>
                                </div>
                            </div>
                            
                            <div className="bg-slate-900/30 border border-slate-800 p-4 rounded-lg">
                                <span className="text-xs text-slate-500 font-mono">LAST VISIT DETECTED</span>
                                <div className="text-sm md:text-base text-white font-mono mt-1">
                                    {new Date(localData.analytics.lastVisit).toLocaleString()}
                                </div>
                            </div>

                             <div className="bg-red-900/10 border border-red-900/30 p-4 rounded-lg mt-8">
                                <h4 className="text-red-400 font-bold text-sm mb-2">Reset Data</h4>
                                <p className="text-xs text-red-200/50 mb-4">Warning: This will clear all visitor statistics collected so far.</p>
                                <button 
                                    onClick={() => {
                                        if(confirm("Reset all analytics data?")) {
                                            setLocalData({ ...localData, analytics: { totalVisits: 0, lastVisit: new Date().toISOString(), devices: { mobile: 0, desktop: 0 } } })
                                        }
                                    }}
                                    className="px-3 py-2 bg-red-900/30 hover:bg-red-800/50 text-red-300 rounded text-xs border border-red-800 transition-colors"
                                >
                                    Clear Analytics
                                </button>
                             </div>
